...
namespace HotelesAPI.Controllers
{
    ...
    public class B2BController : ControllerBase
    {
        ...
        // GET api/b2b/disponibilidad?checkIn=2026-03-01&checkOut=2026-03-05&capacidad=2
                                               ...
                                               [FromQuery] int capacidad = 1)
        {
            ...
            if (agencia == null)
                return Unauthorized(JsonResponse.Error("Token inválido o agencia no autorizada"));

            try
            {
                if (checkIn >= checkOut)
                    return BadRequest(JsonResponse.Error("Fechas inválidas"));
        ...
        public IActionResult Reservar([FromBody] ReservacionB2BDto dto)
        {
            ...
            try
            {
                ...
                if (conflicto)
                    return Conflict(JsonResponse.Error("La habitación no está disponible en esas fechas"));
                ...
                if (habitacion.Estado != "Disponible")
                    return Conflict(JsonResponse.Error("La habitación no está disponible"));
                ...
                // Sincronizar inventario - marcar habitación como Ocupada
                _habitacionDAO.CambiarEstado(dto.IdHabitacion, "Ocupada");

                return StatusCode(201, JsonResponse.Ok("Reservación B2B creada exitosamente", new
                {
                    idReservacion = id,
                    agencia = agencia.Nombre,
        ...
        public IActionResult CancelarReserva(int id)
        {
            ...
            try
            {
                ...
                if (reserva.Estado == "Cancelada")
                    return BadRequest(JsonResponse.Error("La reserva ya está cancelada"));

                _reservacionDAO.Cancelar(id);

                // Sincronizar inventario - liberar habitación
                _habitacionDAO.CambiarEstado(reserva.IdHabitacion, "Disponible");

                return Ok(JsonResponse.Ok("Reservación B2B cancelada y habitación liberada", null));
            }
            catch (Exception ex)
            {
                return StatusCode(500, JsonResponse.Error(ex.Message));
