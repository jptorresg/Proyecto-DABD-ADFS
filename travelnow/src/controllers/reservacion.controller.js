...
const crear = async (req, res) => {
    ...
    if (!tipo || !metodo_pago) return err(res, 'Faltan campos requeridos', 400);
    ...
    try {
        ...
        const [resResult] = await conn.query(
            'INSERT INTO reservacion (codigo_reserva, id_usuario, tipo, total, moneda, estado, metodo_pago, datos_cobro) VALUES (?, ?, ?, 0, "USD", "pendiente", ?, ?)',
        ...
        if ((tipo === 'vuelo' || tipo === 'paquete') && vuelo) {
            ...
            if (!vuelo.pasajeros || !vuelo.pasajeros.length) throw new Error('Se requiere al menos un pasajero');
            ...
            const respVuelo = await proveedorService.reservarVuelo(vuelo.id_proveedor, {
                ...
                metodo_pago:        'agencia',
                id_usuario_externo: usuario.id_usuario,  // requerido por la API Java
                pasajeros:          vuelo.pasajeros,
            });

            // La aerolínea devuelve el objeto desenvuelto (ya lo hace reservarVuelo)
            const codigoVueloProv = String(
                respVuelo?.codigoReservacion || respVuelo?.idReservacion || ''
            );
            ...
            await conn.query(
                'INSERT INTO detalle_vuelo (id_reservacion, id_proveedor, codigo_reserva_proveedor, codigo_vuelo, origen, destino, fecha_salida, tipo_asiento, num_pasajeros, precio_unitario_proveedor, porcentaje_ganancia, precio_total, es_regreso, datos_pasajeros) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?)',
                [idReservacion, vuelo.id_proveedor, codigoVueloProv, vuelo.codigo_vuelo || '', vuelo.origen || '', vuelo.destino || '', vuelo.fecha_salida || null, vuelo.tipo_asiento || 'turista', vuelo.pasajeros?.length || 1, vuelo.precio_proveedor, prov.porcentaje_ganancia, precioTotal, JSON.stringify(vuelo.pasajeros || [])]
            );
        }
        ...
        if (tipo === 'vuelo' && vuelo_regreso) {
            ...
            const respReg = await proveedorService.reservarVuelo(vuelo_regreso.id_proveedor, {
            ...
            const codigoReg = String(
                respReg?.codigoReservacion || respReg?.idReservacion || ''
            );
            ...
            await conn.query(
                'INSERT INTO detalle_vuelo (id_reservacion, id_proveedor, codigo_reserva_proveedor, codigo_vuelo, origen, destino, fecha_salida, tipo_asiento, num_pasajeros, precio_unitario_proveedor, porcentaje_ganancia, precio_total, es_regreso, datos_pasajeros) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)',
                [idReservacion, vuelo_regreso.id_proveedor, codigoReg, vuelo_regreso.codigo_vuelo || '', vuelo_regreso.origen || '', vuelo_regreso.destino || '', vuelo_regreso.fecha_salida || null, vuelo_regreso.tipo_asiento || 'turista', vuelo_regreso.pasajeros?.length || 1, vuelo_regreso.precio_proveedor, prov.porcentaje_ganancia, precioReg, JSON.stringify(vuelo_regreso.pasajeros || [])]
            );
        }
        ...
        if ((tipo === 'hotel' || tipo === 'paquete') && hotel) {
            ...
            const respHotel = await proveedorService.reservarHotel(hotel.id_proveedor, {
                ...
                id_usuario_externo: usuario.id_usuario,   // corregido: era literal '&{...}'
                metodo_pago:        'transferencia',
                notas:              `Agencia TravelNow - usuario ${usuario.id_usuario}`,
            });

            const codigoHotelProv = String(
                respHotel?.idReservacion || ''
            );
            ...
            await conn.query(
                'INSERT INTO detalle_hotel (id_reservacion, id_proveedor, codigo_reserva_proveedor, nombre_hotel, ciudad, tipo_habitacion, fecha_checkin, fecha_checkout, num_huespedes, precio_por_noche_proveedor, porcentaje_ganancia, precio_total) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                [idReservacion, hotel.id_proveedor, codigoHotelProv, hotel.nombre_hotel || '', hotel.ciudad || '', hotel.tipo_habitacion || 'doble', hotel.fecha_checkin, hotel.fecha_checkout, hotel.num_huespedes, hotel.precio_noche_proveedor, prov.porcentaje_ganancia, montoHotel]
            );
        }

        // ── Confirmación ───────────────────────────────────────
        // corregido: tabla era 'reservaciones', debe ser 'reservacion'
        await conn.query(
        ...
        const [rows] = await db.query(
            'SELECT r.*, u.correo, u.nombre, u.apellido FROM reservacion r JOIN usuario u ON u.id_usuario = r.id_usuario WHERE r.id_reservacion = ?',
            [idReservacion]
        );
        const reservacion = rows[0];
        const pdfPath = await generarPDF(reservacion, idReservacion);
        await db.query(
        ...
        await sendMail({
            ...
            subject: `Confirmacion de reserva ${codigoReserva}`,   // corregido: backtick
            html:    `<h2>¡Tu reserva está confirmada!</h2><p>Hola ${reservacion.nombre}, tu reservación <strong>${codigoReserva}</strong> ha sido confirmada por un total de <strong>$${totalAgencia.toFixed(2)} USD</strong>.</p><p>Puedes descargar tu comprobante desde tu historial de reservas.</p>`,
        }).catch(() => {});
        ...
        await conn.rollback();
        return err(res, `Error al crear reservación: ${e.message}`);  // corregido: backtick
    } finally {
        conn.release();
    }
};
...
const obtener = async (req, res) => {
    ...
    const usuario = req.user;
    try {
        const [rows] = await db.query(   // corregido: era 'cosnt'
            'SELECT r.*, u.nombre, u.apellido, u.correo FROM reservacion r JOIN usuario u ON u.id_usuario = r.id_usuario WHERE r.id_reservacion = ?',
            [id]
        );
        ...
        if (reservacion.id_usuario !== usuario.id_usuario && usuario.rol !== 'administrador') {
            return err(res, 'Acceso denegado', 403);  // corregido: era 'Degengado'
        }
...
const descargarPDF = async (req, res) => {
    ...
    try {
        ...
        const pdfFile = path.join(pdfDir, `reserva-${reservacion.id_reservacion}.pdf`);
        if (!fs.existsSync(pdfFile)) await generarPDF(reservacion, id);

        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="TravelNow-${reservacion.codigo_reserva}.pdf"`);  // corregido: backtick
...
const cancelar = async (req, res) => {
    ...
    try {
        ...
        await db.query(
            `INSERT INTO cancelacion (id_reservacion, origen, procesado_por, motivo) VALUES (?, 'administrador', ?, ?)`,
            [id, admin.id_usuario, req.body.motivo || 'Cancelado por administrador']
        );

        const [uRows] = await db.query(
            `SELECT u.correo, u.nombre FROM usuario u JOIN reservacion r ON r.id_usuario = u.id_usuario WHERE r.id_reservacion = ?`,
            [id]
        );
        if (uRows.length) {
            await sendMail({
                ...
                html:    `<p>Hola ${uRows[0].nombre}, tu reservación <strong>${reservacion.codigo_reserva}</strong> ha sido cancelada. ${req.body.motivo ? 'Motivo: ' + req.body.motivo : ''} Contáctanos si tienes preguntas.</p>`,
            }).catch(() => {});
        }
...
const historialUsuario = async (req, res) => {
    ...
    try {
        const [rows] = await db.query(
            `SELECT id_reservacion, codigo_reserva, tipo, total, moneda, estado, fecha_reserva, comprobante_pdf FROM reservacion WHERE id_usuario = ? ORDER BY fecha_reserva DESC LIMIT ? OFFSET ?`,
            [usuario.id_usuario, parseInt(limit), parseInt(offset)]
        );
...
// ─────────────────────────────────────────────────────────────
//  GENERAR PDF (helper interno)
// ─────────────────────────────────────────────────────────────
const generarPDF = async (reservacion, idReservacion) => {
    const pdfDir = process.env.PDF_OUTPUT_DIR || path.join(__dirname, '../../public/comprobantes');
    ...
    try {
        ...
        if (vuelos.length) {
            doc.fontSize(14).font('Helvetica-Bold').text('Vuelo');
            doc.moveDown(0.5);
            vuelos.forEach(v => {
                doc.fontSize(11).font('Helvetica');
        ...
        console.error('Error obteniendo detalles para PDF:', e.message);
    }
