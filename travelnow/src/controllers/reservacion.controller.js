...
const crear = async (req, res) => {
    ...
    try { await conn.beginTransaction(); 
        ...
        if ((tipo === 'vuelo' || tipo === 'paquete') && vuelo) {
            ...
            const respVuelo = await proveedorService.reservarVuelo(vuelo.id_proveedor, {id_vuelo: vuelo.id_vuelo, metodo_pago: 'agencia', pasajeros: vuelo.pasajeros});
            const codigoVueloProv = String(respVuelo.data?.idReservacion || respVuelo.idReservacion || respVuelo.codigo || '');
            const precioTotal = proveedorService.calcularPrecioConGanancia(vuelo.precio_proveedor, prov.porcentaje_ganancia);
            totalAgencia += precioTotal;
            await conn.query('INSERT INTO detalle_vuelo (id_reservacion, id_proveedor, codigo_reserva_proveedor, codigo_vuelo, origen, destino, fecha_salida, tipo_asiento, num_pasajeros, precio_unitario_proveedor, porcentaje_ganancia, precio_total, es_regreso, datos_pasajeros) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?)', [idReservacion, vuelo.id_proveedor, codigoVueloProv, vuelo.codigo_vuelo || '', vuelo.origen || '', vuelo.destino || '', vuelo.fecha_salida || null, vuelo.tipo_asiento || 'turista', vuelo.pasajeros?.length || 1, vuelo.precio_proveedor, prov.porcentaje_ganancia, precioTotal, JSON.stringify(vuelo.pasajeros || [])]);
        }
        //------------- Vuelo Regreso ------------
        if(tipo === 'vuelo' && vuelo_regreso) {
            const prov = await proveedorService.buscarConfig(vuelo_regreso.id_proveedor);
            const respReg = await proveedorService.reservarVuelo(vuelo_regreso.id_proveedor, {id_vuelo: vuelo_regreso.id_vuelo, metodo_pago: 'agencia', pasajeros: vuelo_regreso.pasajeros});
            const codigoReg = String(respReg.data?.idReservacion || respReg.idReservacion || '');
            const precioReg = proveedorService.calcularPrecioConGanancia(vuelo_regreso.precio_proveedor, prov.porcentaje_ganancia);
            await conn.query('INSERT INTO detalle_vuelo (id_reservacion, id_proveedor, codigo_reserva_proveedor, codigo_vuelo, origen, destino, fecha_salida, tipo_asiento, num_pasajeros, precio_unitario_proveedor, porcentaje_ganancia, precio_total, es_regreso, datos_pasajeros) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)', [idReservacion, vuelo_regreso.id_proveedor, codigoReg, vuelo_regreso.codigo_vuelo || '', vuelo_regreso.origen || '', vuelo_regreso.destino || '', vuelo_regreso.fecha_salida || null, vuelo_regreso.tipo_asiento || 'turista', vuelo_regreso.pasajeros?.length || 1, vuelo_regreso.precio_proveedor, prov.porcentaje_ganancia, precioReg, JSON.stringify(vuelo_regreso.pasajeros || [])]);
        }
        // ----------- Reservar Hotel -----------
        if((tipo === 'hotel' || tipo === 'paquete') && hotel) {
           ...
           totalAgencia += totalHotel > 0 ? totalHotel : precioNocheAgencia;
    ...
    try {
      const [rows] = await db.query('SELECT r.*, u.nombre, u.apellido, u.correo FROM reservacion r JOIN usuario u ON u.id_usuario = r.id_usuario WHERE r.id_reservacion = ?', [id]);
        ...
        if(reservacion.id_usuario !== usuario.id_usuario && usuario.rol !== 'administrador') {
            return err(res, 'Acceso Degengado', 403);
        }
...
const descargarPDF = async (req, res) => {
    ...
    try{
        ...
        if(!fs.existsSync(pdfFile)) {
            await generarPDF(reservacion, id);
        }
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', 'attachment; filename="TravelNow-${reservacion.codigo_reserva}.pdf"');
        fs.createReadStream(pdfFile).pipe(res);
    } catch (e) { return err(res, e.message); };
};

// --------------Borrar Reservación (unicamente administrador) -------------------------
const cancelar = async (req, res) => {
    ...
    try {
      ...
      await db.query(
        'UPDATE reservacion SET estado = "cancelada" WHERE id_reservacion = ?', [id]
      );
      await db.query(
      ...
      const [uRows] = await db.query(
        `SELECT u.correo, u.nombre FROM usuario u
  ...
  const historialUsuario = async (req, res) => {
    ...
    try {
      const [rows] = await db.query(
        `SELECT id_reservacion, codigo_reserva, tipo, total, moneda,
                estado, fecha_reserva, comprobante_pdf
         FROM reservacion
         WHERE id_usuario = ?
         ORDER BY fecha_reserva DESC LIMIT ? OFFSET ?`,
  ...
  const generarPDF = async (reservacion, idReservacion) => {
    ...
    doc.fontSize(24).font('Helvetica-Bold').text('TravelHub', { align: 'center' });
    ...
    try {
      const [vuelos] = await db.query(
        'SELECT * FROM detalle_vuelo WHERE id_reservacion = ?', [idReservacion]
      );
      if (vuelos.length) {
      ...
      const [hoteles] = await db.query(
        'SELECT * FROM detalle_hotel WHERE id_reservacion = ?', [idReservacion]
      );
      if (hoteles.length) {
    ...
    doc.fontSize(9).fillColor('gray')
       .text('Comprobante oficial de reserva TravelHub.', { align: 'center' });
