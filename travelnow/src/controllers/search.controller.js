...
const getCities = async (req, rest) => {
    try{
...
const searchFlights = async (req, res) => {
    ...
    try {
        ...
        const vuelos = resultados.filter(r => r.status === 'fulfilled').flatMap(r => r.value).sort((a, b) => a.precio_total - b.precio_agencia);
...
const registrarBusqueda = async (req, tipo, datos) => {
    try {
        ...
        await db.query('INSERT INTO historial_busqueda (id_usuario, tipo_busqueda, origen_busqueda, origen, detino, ciudad, fecha_inicio, fecha_fin, num_pasajeros) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [idUsuario, tipo, origenBusqueda, datos.origen || null, datos.destino || null, datos.ciudad || null, datos.fecha_inicio || null, datos.fecha_fin || null, datos.num_pasajeros || null]);
